...
func registerReceiptRoutes(app *fiber.App, deps protectedDeps) {
	...
	landlord.Get("/receipts/:number.pdf", func(c *fiber.Ctx) error {
		...
		if err != nil || land.ID != row.LandlordID {
...
func buildReceiptPDF(r receiptRow) *gofpdf.Fpdf {
	...
	pdf.Ln(6)
	if r.Method.Valid {
		pdf.Cell(40, 8, fmt.Sprintf("Method: %s", r.Method.String))
		pdf.Ln(6)
	}
	if r.Reference.Valid {
		pdf.Cell(40, 8, fmt.Sprintf("Reference: %s", r.Reference.String))
		pdf.Ln(6)
	}
	pdf.Ln(6)
